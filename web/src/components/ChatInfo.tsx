...
export default function ChatInfo({ chat, onClose }: Props) {
  ...
  const { leaveChat, onlineUsers, pinnedMessages, activeChatId, fetchChats, uploadFile } = useChatStore();
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-dark-bg min-w-0" onClick={() => setMemberMenu(null)}>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        ...
        {isGroup && (
          <div className="flex justify-center gap-2.5 px-5 pb-5">
            <ActionBtn icon={<IconUserPlus />} label="добавить" onClick={() => setShowAdd(true)} />
            <ActionBtn icon={<IconBell />} label="уведомл." onClick={() => {}} />
            <ActionBtn icon={<IconSettings />} label="настройки" onClick={() => setShowEditModal(true)} />
            <ActionBtn icon={<IconDotsH />} label="ещё" onClick={() => {}} />
          </div>
        )}
      ...
      <AddMembersModal open={showAdd} onClose={() => setShowAdd(false)} chatId={chat.chat.id} existing={chat.members} onAdded={fetchChats} />
...
function ActionBtn({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className="flex flex-col items-center justify-center gap-1.5 min-h-[44px] w-[72px] py-2.5 rounded-[12px] bg-surface dark:bg-dark-elevated hover:bg-surface-light dark:hover:bg-dark-hover transition-colors group">
      <span className="text-txt-secondary dark:text-[#8b98a5] group-hover:text-primary transition-colors">{icon}</span>
      <span className="text-[10px] font-medium text-txt-secondary dark:text-[#8b98a5] group-hover:text-txt dark:group-hover:text-[#e7e9ea] transition-colors">{label}</span>
    </button>
  );
}
  ...
  return (
    <div className="flex items-center gap-3 px-2 py-2.5 min-h-[44px] rounded-compass hover:bg-surface dark:hover:bg-dark-hover transition-colors cursor-pointer"
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-txt-secondary dark:text-[#8b98a5]">Пользователь Мессенджера</p>
      </div>
      {role && (
